package ge.tastyerp.config.infrastructure.firebase;
 ...
 * exchange, which otherwise lands on the first user request (e.g. the audit
 * dashboard fetching customers). A single trivial read moves that cost to
 * startup.
 *
 * Runs on a background thread after ApplicationReadyEvent so it never delays
 * startup or health checks; failure is logged and ignored.
 */
@Slf4j
@Component
...
public class FirestoreWarmup {
    ...
    public void warmUp() {
        Thread warmupThread = new Thread(() -> {
            ...
            try {
                firestore.collection("customers").limit(1).get().get();
